...
class FSD_Cal:
    def __init__(self,
                 ...
                 file_name='fsd.csv',
                 tmp_file_name='long_short_depth_cunt.csv',
                 zfile_name='zfsd.csv'):
    ...
    def stat_calculate(self):
        ...
        for s in set(self.sample_name_list):
            if s + '.L+'+str(self.n) and s + '.S'+str(self.n) in self.all_data.columns:
                sub = pd.DataFrame({
                    s:
                    self.all_data[s + f'.L{self.n}'] / self.all_data[s + f'.S{self.n}']
                })
                m_list.append(sub)
            t += 1
    ...
    def file_concate(self):
        ...
        for n in specific_file_list:
            if os.path.getsize(n) > 0:
                ...
                df = pd.read_csv(n, sep='\t', header=None, index_col=False)
        ...
        self.all_data = reduce(
            lambda left, right: pd.merge(left, right, on='pos'), merge_list)
        self.tmp_file_save_path = os.path.join(
            self.sample_path, self.target_path, self.tmp_file_name)
    ...
    def runAll(self,bz,bs,bp,exp):
        self.file_concate()
        fsd_feature_get = fsd_feature_cal.ZFSD_Feature_Cal(self.sample_path,self.target_path,bz,bs,bp,exp)
        fsd_feature_get.run()
...
if __name__ == '__main__':
    ...
    #n = 125
    concating_file = FSD_Cal(sample_path)
