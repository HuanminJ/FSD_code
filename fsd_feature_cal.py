...
class ZFSD_Feature_Cal:
    def __init__(self,
                 ...
                 baseline_zfsd,
                 baseline_smothed,
                 baseline_peaks_feature,
                 exp,
                 zfsd="zfsd.csv"
                 ):
        self.root_dir = root_dir
        self.target_path = target_path
        self.zfsd_path = os.path.join(root_dir, target_path, zfsd)
    ...
    def peaks(self,
        ...
        if self.exp:
            baseline_peaks = pd.read_csv(
                self.baseline_peaks_feature, index_col=False)
            peaks_path = self.target_path + "/FSD/peaks"
            corr_path = self.target_path + "/FSD/corr"
            smothed = []
            feature_exp = []
            feature_s = []
            feature_n = []
            ...
            for s in dft.columns:
                ...
                name = s
                y = savgol_filter(
                    dft[s], 51, 1, mode="nearest")  # 根据均值确定平滑曲线参数第一个参数51
            ...
            data_peaks = reduce(
                lambda i, j: pd.merge(i, j, on="pos", how="outer"),
                feature_exp)
            os.makedirs(peaks_path, exist_ok=True)
            data_peaks.to_csv(
                os.path.join(self.root_dir, peaks_path, out1), index=False)
            data_filtered = pd.concat(smothed, axis=1)
            data_filtered.to_csv(
                os.path.join(self.root_dir, peaks_path, out2), index=False)

            data_s = reduce(
                lambda i, j: pd.merge(i, j, on="spos", how="outer"), feature_s)
            data_n = reduce(
                lambda i, j: pd.merge(i, j, on="dpos", how="outer"), feature_n)
            data_s.to_csv(
                os.path.join(self.root_dir, peaks_path, out3), index=False)
            data_n.to_csv(
                os.path.join(self.root_dir, peaks_path, out4), index=False)

            df_s = (pd.DataFrame.from_dict(
                s_cunts, orient="index", columns=[
                    "similar_peak_cunts"
                ]).reset_index().rename(columns={"index": "sample"}))
            df_n = (pd.DataFrame.from_dict(
                d_cunts, orient="index", columns=[
                    "different_peak_cunts"
                ]).reset_index().rename(columns={"index": "sample"}))
            df_cunts = (pd.DataFrame.from_dict(
                peaks_cunt, orient="index", columns=[
                    "peak_counts"
                ]).reset_index().rename(columns={"index": "sample"}))
            ...
            df_sn_cunt = reduce(lambda x, y: pd.merge(x, y, on="sample"),
                                cunt_list)
            df_sn_cunt.to_csv(
                os.path.join(self.root_dir, peaks_path, out5), index=False)

            # 计算实验样本与baseline样本平滑后zfsd曲线相关性
            base_smothed = pd.read_csv(
                self.baseline_smothed, usecols=["Mean"], index_col=False)
            os.makedirs(corr_path, exist_ok=True)
            pearson = {}
            for col in data_filtered.columns:
                pearson[col] = base_smothed["Mean"].corr(data_filtered[col])
            pd.DataFrame.from_dict(
                pearson, orient="index", columns=["pearson"]).to_csv(
                    os.path.join(self.root_dir, corr_path, out6))
            print("peaks feature stat done!")
            print("person corr results saved!")
        else:
            print("compute baseline peaks feature!")
            dft = pd.read_csv(self.zfsd_path, index_col=False)
            peaks_path = self.target_path + "/FSD/peaks"
            smothed = []
            feature_exp = []
            ...
            data_filtered.to_csv(
                os.path.join(self.root_dir, peaks_path, out2), index=False)
            print("compute baseline peaks feature done!")
    ...
    def euclidean_distance(
        ...
        ed_path = self.target_path + "/FSD/ED"
        ...
        df_ou_total.to_csv(
            os.path.join(self.root_dir, ed_path, output1), index=False)
        df_ou_255.to_csv(
            os.path.join(self.root_dir, ed_path, output2), index=False)
        df_ex = pd.DataFrame({"sample_excluded": sample_excl})
        df_ex.to_csv(
            os.path.join(self.root_dir, ed_path, output3), index=False)
        print("euclidean_distance calculate done,results saved~")

    def audc(self, output1="255_audc_posi.csv", output2="255_audc_nega.csv"):
        # 计算曲线下面积
        audc_path = self.target_path + "/FSD/AUDC"
        dft = pd.read_csv(self.zfsd_path, index_col=False)
        ...
        df_audc_dic_255_pos.to_csv(
            os.path.join(self.root_dir, audc_path, output1), index=False)
        df_audc_dic_255_nega.to_csv(
            os.path.join(self.root_dir, audc_path, output2), index=False)
...
if __name__ == "__main__":
    ...
    fsd_feature_get = ZFSD_Feature_Cal(root_dir, target_path)
    fsd_feature_get.run()
